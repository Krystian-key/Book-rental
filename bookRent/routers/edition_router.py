...
def get_by_id(edition_id: int, db: Session = Depends(get_db)):
    return try_perform(get_edition_by_id, edition_id, db)
...
def get_by_book_id(book_id: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_book_id, book_id, db)
...
def get_by_edition_num(num: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_edition_number, num, db)
...
def get_by_original_series(series: str, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_original_series, series, db)
...
def get_by_author_id(author_id: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_author_id, author_id, db)
...
def get_by_author_name(name: str, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_author_name, name, db)
...
def get_by_author_surname(surname: str, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_author_surname, surname, db)
...
def get_by_author_birth_year(birth: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_author_birth_year, birth, db)
...
def get_by_author_death_year(death: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_author_death_year, death, db)
...
def get_by_illustrator_id(illustrator_id: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_illustrator_id, illustrator_id, db)


@router.get("/get-by-illustrator-name", response_model=Edition | list[Edition] | None)
def get_by_author_name(name: str, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_illustrator_name, name, db)
...
def get_by_illustrator_surname(surname: str, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_illustrator_surname, surname, db)
...
def get_by_illustrator_birth_year(birth: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_illustrator_birth_year, birth, db)
...
def get_by_illustrator_death_year(death: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_illustrator_death_year, death, db)
...
def get_by_translator_id(translator_id: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_translator_id, translator_id, db)
...
def get_by_translator_name(name: str, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_translator_name, name, db)
...
def get_by_translator_surname(surname: str, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_translator_surname, surname, db)
...
def get_by_translator_birth_year(birth: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_translator_birth_year, birth, db)
...
def get_by_translator_death_year(death: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_translator_death_year, death, db)
...
def get_by_original_language_id(lang_id: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_original_language_id, lang_id, db)
...
def get_by_original_language_name(lang: str, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_original_language, lang, db)
...
def get_by_edition_language_id(lang_id: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_edition_language_id, lang_id, db)
...
def get_by_edition_language_name(lang: str, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_edition_language, lang, db)
...
def get_by_language_id(lang_id: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_language_id, lang_id, db)
...
def get_by_language_name(lang: str, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_language, lang, db)
...
def get_by_publisher_id(publisher_id: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_publisher_id, publisher_id, db)
...
def get_by_publisher_name(publisher_name: str, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_publisher_name, publisher_name, db)
...
def get_by_publisher_city(city: str, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_publisher_city, city, db)
...
def get_by_publisher_foundation_year(year: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_publisher_foundation_year, year, db)
...
def get_by_form_id(form_id: int, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_form_id, form_id, db)
...
def get_by_form_name(form: str, db: Session = Depends(get_db)):
    return try_perform(get_editions_by_form, form, db)