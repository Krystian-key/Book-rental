...
def add_person(name: str, surname: str,
               ...
               force_add = False, db: Session = Depends(get_db())):
    if not force_add:
...
def add_publisher(name: str, localization: str, foundation_year: int, force_add = False, db: Session = Depends(get_db())):
    if not force_add:
        existing_publisher = db.query(Publisher).filter_by(name=name).first()
...
def add_language(language: str, force_add = False, db: Session = Depends(get_db())):
    if not force_add:
        existing_language = db.query(Language).filter_by(name=language).first()
...
def add_form(form: str, force_add = False, db: Session = Depends(get_db())):
    if not force_add:
        existing_form = db.query(Form).filter_by(form=form).first()
        if existing_form:
...
def add_category(category: str, force_add = False, db: Session = Depends(get_db())):
    if not force_add:
...
def add_book(original_title: str, original_lang: str, original_series: Optional[str],
             ...
             categories: List[str], force_add = False, db: Session = Depends(get_db())):
    messages = []

    author = db.query(Person).filter_by(
        name=author_name,
    ...
    language = db.query(Language).filter_by(name=original_lang).first()
    if not language:
        messages.append(add_language(original_lang, True, db)['message'])
        language = db.query(Language).filter_by(name=original_lang).first()

    if not force_add:
        existing_book = db.query(Book).filter_by(
    ...
    book = db.query(Book).filter_by(original_title=original_title, author_id=author.id).first()
    for category in categories:
        existing_category = db.query(Category).filter_by(category=category).first()
        if not existing_category:
            messages.append(add_category(category, True, db))
            existing_category = db.query(Category).filter_by(category=category).first()
...
def add_edition_info(original_title: str, original_lang: str, original_series: Optional[str],
    ...
    if not force_add:
        existing_edition = db.query(EditionInfo).filter_by(isbn=isbn).first()
        if existing_edition:
    ...
    illustrator = db.query(Person).filter_by(
        name=illustrator_name,
        surname=illustrator_surname
    ).first()
    if not illustrator:
        messages.append(add_person(illustrator_name, illustrator_surname, force_add=True, db=db)['message'])
        illustrator = db.query(Person).filter_by(
            name=illustrator_name,
            surname=illustrator_surname
        ).first()

    translator = db.query(Person).filter_by(
    ...
    ed_lang = db.query(Language).filter_by(name=edition_lang).first()
    if not ed_lang:
        messages.append(add_language(edition_lang, True, db=db)['message'])
        ed_lang = db.query(Language).filter_by(name=edition_lang).first()

    publisher = db.query(Publisher).filter_by(name=publisher_name).first()
    if not publisher:
        raise ValueError(f"Wydawca {publisher_name} nie istnieje.")

    ed_form = db.query(Form).filter_by(name=form).first()
    if not ed_form:
        messages.append(add_form(form, True, db=db)['message'])
        ed_form = db.query(Form).filter_by(name=form).first()
...
def add_copy(original_title: str, original_lang: str, original_series: Optional[str],
    ...
    edition = db.query(EditionInfo).filter_by(isbn=isbn).first()
    if not edition:
        messages.append(
            add_edition_info(original_title, original_lang, original_series,
                             ...
                             form, isbn, ukd, categories, db=db
        )['messages'])
        edition = db.query(EditionInfo).filter_by(isbn=isbn).first()
...
def add_annotation(book_id: Optional[int], edition_id: Optional[int], copy_id: Optional[int],
    ...
    if edition_id:
        ...
        word = "wydania"
        item_id = edition_id
        edition = db.query(EditionInfo).filter_by(id=edition_id).first()
        if not edition:
    ...
    return {"message": try_commit(
        db,
        f"Dodano adnotację do {word} {item_id}",
